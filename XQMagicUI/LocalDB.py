...
# ----------------------------------------------------------------
# python -m pwiz -e sqlite path/to/sqlite_database.db > 要生成的python文件名称
...
class MoveBookMixIn:
    ...
    def getRecord(self, fen, iccs=None):
        ...
        for b, b_state in [(board, ""), (board.mirror(), "mirror")]:
            if iccs:
                query = (
                    self.book_cls.select()
                    .where(self.book_cls.key == b.zhash(), self.book_cls.iccs == iccs)
                    .order_by(-self.book_cls.score)
                )
            else:
                query = (
                    self.book_cls.select()
                    .where(self.book_cls.key == b.zhash())
                    .order_by(-self.book_cls.score)
                )
...
class MasterBook:
    ...
    def getRecords(self, board):
        for b, b_state in [(board, ""), (board.mirror(), "mirror")]:
            query = (
                EvPosition.select()
                .where(EvPosition.key == b.zhash())
                .order_by(-EvPosition.score)
            )
            query.execute()
            if query.count() > 0:
                ret = list(query)
    ...
    def saveRecord(self, fen, step, score, actions):
        ...
        if item is None:
            ...
            if record_state == "mirror":
                # 查到的数据与本次保存的相比是镜像移动，本次移动镜像化
                new_actions = {
                    cchess.iccs_mirror(iccs): act for iccs, act in actions.items()
                }
            else:
                new_actions = actions
...
class LocalBook:
    ...
    def changeBookmarkName(self, old_name, new_name):
        query = (
            Bookmark.update(name=new_name).where(Bookmark.name == old_name).execute()
        )
        return True
    ...
    def getMoves(self, fen):
        ...
        f_mirror = cchess.fen_mirror(fen)
        query = (
            Book.select().where((Book.fen == fen) | (Book.fen == f_mirror)).execute()
        )
...
class OpenBookYfk(OpenBookDB):
    ...
    def getMoves(self, fen):
        ...
        query = (
            Bhobk.select()
            .where(
                ((Bhobk.vkey == zhash) | (Bhobk.vkey == zhash_mirror))
                & (Bhobk.vvalid == 1)
            )
            .order_by(-Bhobk.vscore)
            .execute()
        )
...
class OpenBookPF(OpenBookDB):
    ...
    def getMoves(self, fen):
        ...
        query = (
            PfBook.select()
            .where(
                ((PfBook.vkey == zhash) | (PfBook.vkey == zhash_mirror))
                & (PfBook.vvalid == 1)
            )
            .order_by(-PfBook.vscore)
            .execute()
        )
