...
from .Utils import ThreadRunner, TimerMessageBox, scaleImage
...
class MovieSource:
    ...
    def get_image_roi(self, roi_rect=None):
        ...
        if roi_rect is not None:
            img_roi = cv.split(
                frame[roi_rect[0][1] : roi_rect[1][1], roi_rect[0][0] : roi_rect[1][0]]
            )[0]
        ...
        for x in range(25):
            ...
            if roi_rect is not None:
                img_new_roi = cv.split(
                    new_frame[
                        roi_rect[0][1] : roi_rect[1][1], roi_rect[0][0] : roi_rect[1][0]
                    ]
                )[0]
...
class OnlineManager(QObject):
    ...
    def detect_board(self):
        ...
        roiRect = QRect(
            self.roi_pos.x, self.roi_pos.y, self.roi_size.width, self.roi_size.height
        )
    ...
    def match_board(self, board, is_flip=False):
            ...
            for p in self.piece_points:
                if rect.contains(p.x, p.y):
                    ...
                    if fench not in self.piece_tmpl:
                        self.piece_tmpl[fench] = self.get_piece_img(
                            pt, gray=False, small=False
                        )
        ...
        for y in range(10):
            for x in range(9):
                ...
                if max_match > 0.9:
                    img_src = cv.putText(
                        img_src, name, tuple(pos), cv.FONT_HERSHEY_SIMPLEX, 1, color, 2
                    )
    ...
    def to_schema(self, name):
        ...
        templ["roi"] = [
            self.roi_pos.x / base,
            self.roi_pos.y / base,
            self.roi_size.width / base,
            self.roi_size.height / base,
        ]

        templ["board"] = [
            self.board_pos.x / base,
            self.board_pos.y / base,
            self.board_size.width / base,
            self.board_size.height / base,
        ]
    ...
    def load_schema_image(self, name):
        ...
        image = cv.imdecode(
            np.fromfile(file=str(file_name), dtype=np.uint8), cv.IMREAD_COLOR
        )
        if image is None:
            return False
...
class OnlineDialog(QDialog):
    ...
    def showEvent(self, event):
        if self.manager.source.is_connected():
            ...
            self.capture_marge = Point(
                inner_rect.x() - win_rect.x(), inner_rect.y() - win_rect.y()
            )
