...
class ChessBoardBaseWidget(QWidget):
    ...
    def scaleBoard(self, scale):
        ...
        if not self.use_svg:
            ...
            self._board_img = SvgToPixmap(
                self.base_board, self.board_width, self.board_height
            )
            ...
            for name in piece_names:
                ...
                self.pieces_img[name] = SvgToPixmap(
                    svg, self.piece_size, self.piece_size
                )
    ...
    def resizeBoard(self, size):
        ...
        new_scale = min(
            (new_width - 10) / self.base_board_width,
    ...
    def getBoardRect(self):
        return QRect(
            self.board_start_x, self.board_start_y, self.board_width, self.board_height
        )
    ...
    def board_to_view(self, x, y, bias=0):
        ...
        board_y = (
            self.board_start_y + self.offset_y + self.border_y + (9 - y) * self.space_y
        )
...
class ChessBoardWidget(ChessBoardBaseWidget):
    ...
    def paintEvent(self, ev):
        ...
        if len(self.move_steps_show) > 0:
            ...
            painter.drawPixmap(
                step_point[0], step_point[1], self.pieces_img[piece.get_color_fench()]
            )
    ...
    def make_show_steps(self, p_from, p_to, step_diff):
        ...
        step = (
            (board_p_to[0] - board_p_from[0]) // step_diff,
...
class ChessBoardEditWidget(ChessBoardBaseWidget):
    ...
    def resizeEvent(self, ev):
        ...
        self.piece_off_pos = [
            (self.board_start_x - self.piece_size - 20, self.board_start_y),
        ...
        for index, name in enumerate(piece_names):
            ...
            pos_y = self.piece_off_pos[col_index][1] + int(
                self.piece_size * 1.2 * ((index % 7) + 1)
            )
    ...
    def mousePressEvent(self, mouseEvent):
        ...
        if self.selected_name:
            self.selected_pos = QPoint(
                pos.x() - self.piece_size // 2, pos.y() - self.piece_size // 2
            )
    ...
    def sizeHint(self):
        return QSize(
            int(self.base_board_width / 9 * 11) + 50, self.base_board_height + 10
        )
